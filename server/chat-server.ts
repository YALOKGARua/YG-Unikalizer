    ...
    for (const fp of files) {
      try {
        ...
        for (const ln of lines) {
          ...
          if (!process.env[key]) (process.env as any)[key] = val
        }
      } catch {}
    }
  }
...
try {
  ...
  if (fs.existsSync(LOG_FILE)) {
    try {
      ...
      for (const ln of lines) {
        try { const obj = JSON.parse(ln); if (obj && obj.type === 'admin_event') allEvents.push(obj) } catch {}
      }
    } catch {}
  }
} catch {}
...
type ExtWebSocket = WebSocket & { id?: string; isAlive?: boolean; isAdmin?: boolean; user?: any }
...
function broadcast(obj: any) {
  ...
  for (const client of wss.clients) {
    if ((client as any).readyState === 1) {
      try { (client as any).send(data) } catch {}
    }
  }
}

function sendAdmins(obj: any) {
  const data = JSON.stringify(obj)
  for (const client of wss.clients) {
    const c = client as ExtWebSocket
    if ((c as any).readyState === 1 && c.isAdmin) {
      try { (c as any).send(data) } catch {}
    }
  }
  if (obj && obj.type === 'admin_event') {
    try { allEvents.push(obj) } catch {}
    try { fs.appendFile(LOG_FILE, JSON.stringify(obj) + '\n', () => {}) } catch {}
  }
}

function broadcastStats() {
  const count = Array.from(wss.clients).filter(c => (c as any).readyState === 1).length
...
wss.on('connection', (ws: ExtWebSocket, req) => {
  ...
  ws.isAlive = true
  ws.isAdmin = false
  ws.user = { id: ws.id, name: '', connectedAt: Date.now(), lastSeen: Date.now(), lastMessageAt: 0, messageCount: 0, ip: (req && (req.socket as any) && (req.socket as any).remoteAddress) ? (req.socket as any).remoteAddress : '', userAgent: (req && (req.headers as any) && (req.headers as any)['user-agent']) ? String((req.headers as any)['user-agent']) : '', url: (req && (req as any).url) ? String((req as any).url) : '' }
  ...
  const exportUser = (c: ExtWebSocket) => ({ id: c.user && c.user.id ? c.user.id : c.id, name: c.user && c.user.name ? c.user.name : '', ip: c.user && c.user.ip ? c.user.ip : '', userAgent: c.user && c.user.userAgent ? c.user.userAgent : '', url: c.user && c.user.url ? c.user.url : '', connectedAt: c.user && c.user.connectedAt ? c.user.connectedAt : 0, lastSeen: c.user && c.user.lastSeen ? c.user.lastSeen : 0, lastMessageAt: c.user && c.user.lastMessageAt ? c.user.lastMessageAt : 0, messageCount: c.user && typeof c.user.messageCount === 'number' ? c.user.messageCount : 0, state: (c as any).readyState, alive: !!c.isAlive, geo: c.user && c.user.geo ? c.user.geo : null })
  ...
  ws.on('message', (data: any) => {
    ...
    if (type === 'admin_subscribe') {
      ...
      if (ADMIN_PASSWORD && pass !== ADMIN_PASSWORD) { try { send(ws, { type: 'admin_users', ok: false, error: 'forbidden' }) } catch {} ; return }
      ws.isAdmin = true
      const users = Array.from(wss.clients).map(c => exportUser(c as ExtWebSocket))
  ...
  try { ws.send(JSON.stringify({ type: 'stats', count: Array.from(wss.clients).filter(c => (c as any).readyState === 1).length })) } catch {}
  ws.on('close', () => { try { sendAdmins({ type: 'admin_event', kind: 'disconnected', ts: Date.now(), user: exportUser(ws) }) } catch {} })
})

const interval = setInterval(() => {
  for (const ws of wss.clients) {
    const c = ws as ExtWebSocket
    if (c.isAlive === false) { try { (c as any).terminate() } catch {} ; continue }
    c.isAlive = false
    try { (c as any).ping() } catch {}
  }
}, 30000)

wss.on('close', () => { clearInterval(interval) })
wss.on('headers', () => { setTimeout(broadcastStats, 0) })
wss.on('connection', (ws) => { (ws as any).on('close', () => { setTimeout(broadcastStats, 0) }) })