      ...
      exec('netstat -ano -p tcp', { windowsHide: true }, (err, stdout) => {
        ...
        if (stdout) {
          ...
          for (const ln of lines) {
            ...
            const mPort = ln.match(/\s(?:0\.0\.0\.0|127\.0\.0\.1|\[::\]|\[::1\]:(\d+)|:(\d+))/)
            const mPid = ln.match(/\s(\d+)\s*$/)
            if (mPort) {
              const prt = Number(mPort[1] || mPort[2])
...
app.whenReady().then(() => {
  ...
  let gameServerProcess: any = null
  ...
  const startMobileServerAlternative = (serverPath: string) => {
    try {
      ...
      const nodePathEnv = nodeModulesCandidates.filter((p) => { try { return fs.existsSync(p) } catch { return false } }).join(path.delimiter)
      const env = { ...process.env, ELECTRON_RUN_AS_NODE: '1', NODE_PATH: nodePathEnv }
      const cwd = path.dirname(serverPath)
      let spawnServerPath = serverPath
      try {
        if (serverPath.includes('app.asar')) {
          const unpacked = path.join(process.resourcesPath || '', 'app.asar.unpacked', 'server', 'mobile-sync-server.js')
          if (fs.existsSync(unpacked)) spawnServerPath = unpacked
        }
      } catch {}
      mobileServerProcess = spawn(nodePath, [spawnServerPath], { detached: false, stdio: ['ignore', 'pipe', 'pipe'], env, cwd, windowsHide: true })
  ...
  const startMobileServer = () => {
    ...
    try {
      ...
      if (isDev) {
        ...
        // Prefer requiring from app.asar to leverage Electron's asar-aware require,
        // then fall back to unpacked/other locations.
        const possiblePaths = [
      ...
      console.log('📱 Server file exists, trying to start...')
  ...
  const startGameServerAlternative = (serverPath: string) => {
    try {
      console.log('🔄 Starting game server as separate process...')
      const { spawn } = require('child_process')
      const nodePath = process.execPath
      const nodeModulesCandidates = [
        path.join(process.resourcesPath || '', 'app.asar', 'node_modules'),
      ...
      try {
        if (serverPath.includes('app.asar')) {
          const unpacked = path.join(process.resourcesPath || '', 'app.asar.unpacked', 'server', 'game-server.js')
          if (fs.existsSync(unpacked)) spawnServerPath = unpacked
        }
      } catch {}
      gameServerProcess = spawn(nodePath, [spawnServerPath], { detached: false, stdio: ['ignore', 'pipe', 'pipe'], env, cwd, windowsHide: true })
      gameServerProcess.stdout.on('data', (data: any) => {
        const line = String(data || '').trim()
      ...
      gameServerProcess.stderr.on('data', (data: any) => {
        const line = String(data || '').trim()
        if (line) console.error('🎮 Game Server Error:', line)
      })
      gameServerProcess.on('close', (code: any) => {
        console.log(`🎮 Game Server process exited with code ${code}`)
        gameServerStarted = false
        gameServerProcess = null
      })
      gameServerProcess.on('error', (error: any) => {
  ...
  const startGameServer = () => {
    ...
    try {
      ...
      if (isDev) {
        serverPath = path.join(__dirname, '..', 'server', 'game-server.js')
      } else {
        const possiblePaths = [
          path.join(process.resourcesPath, 'app.asar', 'server', 'game-server.js'),
          path.join(process.resourcesPath, 'app.asar.unpacked', 'server', 'game-server.js'),
          path.join(process.resourcesPath, 'server', 'game-server.js'),
          path.join(__dirname, '..', 'server', 'game-server.js'),
          path.join(process.cwd(), 'server', 'game-server.js')
        ]
      ...
      if (!fs.existsSync(serverPath)) {
        console.log('🎮 Game server file not found:', serverPath)
        return false
      }
      
      console.log('🎮 Game server file exists, starting...')
  ...
  try {
    ipcMain.handle('game-server-info', async () => {
      try {
        const os = require('os')
  ...
  ipcMain.handle('load-rust-module', async () => {
    try {
      const rustPath = path.join(__dirname, '..', 'rust-native', 'index.node')
  ...
  ipcMain.handle('rust-ahash-batch', async (_e, payload) => {
    try {
  ...
  ipcMain.handle('system-info', async () => {
    try {
      ...
      try {
        const clean = (s: string) => { try { let v = String(s || ''); v = v.replace(/^ANGLE \(([^)]+)\).*$/, '$1'); v = v.replace(/\bDirect3D.*$/i, ''); v = v.replace(/Microsoft Basic Render Driver/ig, ''); v = v.replace(/\s+/g, ' ').trim(); return v } catch { return String(s || '') } }
        if (app.getGPUInfo) {
          const info: any = await app.getGPUInfo('complete')
          const list: any[] = info && Array.isArray(info.gpuDevice) ? info.gpuDevice : []
          const filtered = list.filter((d:any)=>d && !/Microsoft Basic Render Driver/i.test(String(d.deviceString||'')) && !/SwiftShader/i.test(String(d.deviceString||'')))
          const pick = filtered.length ? (filtered.find((d:any)=>d.active) || filtered[0]) : (list.find((d:any)=>d && d.active) || list[0])
          if (pick) {
      ...
      try {
        if ((!gpuName || /unknown/i.test(gpuName)) && process.platform === 'win32') {
          ...
          let data = ''
          ps.stdout && ps.stdout.on('data', c => { data += c.toString() })
          await new Promise(resolve => { ps.on('exit', () => resolve(null)) })
          try {
          ...
          if (!gpuName || /unknown/i.test(gpuName)) {
            ...
            let data2 = ''
            ps2.stdout && ps2.stdout.on('data', c => { data2 += c.toString() })
            await new Promise(resolve => { ps2.on('exit', () => resolve(null)) })
            try {
              const parsed = JSON.parse(data2 || '{}')
              const arr = Array.isArray(parsed) ? parsed : [parsed]
              const firstGood = arr.find((it:any)=>it && it.Name && !/Microsoft Basic Render Driver/i.test(String(it.Name))) || arr[0]
              if (firstGood) {
          ...
          if (!gpuName || /unknown/i.test(gpuName)) {
            try {
              const outPath = path.join(app.getPath('temp'), `dxdiag-${Date.now()}.txt`)
              const dx = spawn('dxdiag', ['/whql:off', '/t', outPath], { windowsHide: true })
              await new Promise(resolve => { dx.on('exit', () => resolve(null)) })
              try {
          ...
          if (!gpuName || /unknown/i.test(gpuName)) {
            try {
              ...
              let out = ''
              reg.stdout && reg.stdout.on('data', c => { out += c.toString() })
              await new Promise(resolve => { reg.on('exit', () => resolve(null)) })
              const lines = String(out||'').split(/\r?\n/)
              const found = lines.map(l=>{ const m = l.match(/DriverDesc\s+REG_SZ\s+(.+)/i); return m && m[1] ? m[1].trim() : '' }).filter(Boolean)
      ...
      const normalizeGpuName = (name: string, vendor?: string) => {
        try {
      ...
      return {
        ok: true,
        os: { platform: os.platform(), type: os.type(), release: os.release(), arch: os.arch(), version: (os as any).version ? (os as any).version() : '' },
