...
contextBridge.exposeInMainWorld('api', {
  ...
  native: {
    ...
    fileAHash: async (filePath: string) => { const mod = loadNative(); if (!mod) return null; const dec = await decodeGray8(filePath); if (!dec) return null; const res = mod.aHashFromGray8(dec.buf, dec.width, dec.height, dec.stride); return typeof res === 'bigint' ? res.toString() : res },
    fileDHash: async (filePath: string) => { const mod = loadNative(); if (!mod) return null; const dec = await decodeGray8(filePath); if (!dec) return null; const res = mod.dHashFromGray8(dec.buf, dec.width, dec.height, dec.stride); return typeof res === 'bigint' ? res.toString() : res },
    filePHash: async (filePath: string) => { const mod = loadNative(); if (!mod) return null; const dec = await decodeGray8(filePath); if (!dec) return null; const res = mod.pHashFromGray8(dec.buf, dec.width, dec.height, dec.stride); return typeof res === 'bigint' ? res.toString() : res },
    gpu: {
