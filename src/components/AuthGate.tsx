...
export default function AuthGate() {
  ...
  useEffect(() => {
    ...
    ;(async () => {
      try {
        ...
        if (!cancelled && r && r.ok) {
          setRequired(!!(r as any).required && !(r as any).authed)
          setAuthed(!!(r as any).authed || !(r as any).required)
        }
      } catch {}
    })()
  ...
  const login = async () => {
    ...
    try {
      ...
      if (r && (r as any).ok && (r as any).authed) {
        setAuthed(true)
      } else {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/80">
      <div className="w-80 glass rounded-xl p-4 border border-white/10 bg-slate-950">
